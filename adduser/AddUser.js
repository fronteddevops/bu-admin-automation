...
(async function AddUser() {
  ...
  try {
    ...
    await emailInputuser.sendKeys("john@example.com");
    ...
    await passwordInputuser.sendKeys("MySecret123");
    ...
    const closeButton = await driver.wait(
        until.elementLocated(
          By.xpath("//button[contains(@class, 'modal-cancel-button') and normalize-space()='Close']")
        ),
        10000
      );
    ...
    const userPrivilegeButton = await driver.wait(
        until.elementLocated(
          By.xpath("//button[@type='button' and contains(@class, 'primary-btn') and normalize-space()='User/Privilege']")
        ),
        10000
      );
    ...
    const addGroupButton = await driver.wait(
        until.elementLocated(
          By.xpath("//button[@type='button' and contains(@class, 'primary-btn') and normalize-space()='Add Group']")
        ),
        10000
      );
      ...
      const groupLabel = await driver.wait(
        until.elementLocated(
          By.xpath("//label[normalize-space()='Group']")
        ),
        10000
      );
      ...
      await groupInput.sendKeys("Admin Group");
    ...
    // Select Option
      ...
      async function selectPermission(sectionText, optionText) {
        const section = await driver.wait(
          until.elementLocated(
            By.xpath(`//p[normalize-space()='${sectionText}']/ancestor::div[contains(@class, 'd-flex')]`)
          ),
          10000
        );
      ...
      await selectPermission("kanban", "Can create");
      ...
      const saveButtonOption = await driver.wait(
        until.elementLocated(
          By.xpath("//button[contains(@class, 'modal-success-button') and normalize-space()='save']")
        ),
        10000
      );
