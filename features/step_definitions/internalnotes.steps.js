...
Given(
  "I am logged in For Internal Notes",
  { timeout: 180000 },
  async function () {
    await driver.get(
...
When("Click on Internal Notes", async function () {
  const NotesButton = await driver.wait(
    until.elementLocated(
      By.xpath("//button[contains(text(), 'INTERNAL NOTES')]")
    ),
    10000
  );
...
When("Enter Title", async function () {
  const titleInput = await driver.findElement(By.id("note-title"));
  await titleInput.clear();
  await titleInput.sendKeys("kanban title");
  await driver.sleep(1000);
});

When("Enter Description", async function () {
  const descriptionTextarea = await driver.findElement(
    By.id("note-description")
  );
  await descriptionTextarea.sendKeys("This is a test note description.");
  await driver.sleep(10000);
});

When("Click on Add Button" , async function(){
  const addButton = await driver.wait(
    until.elementLocated(By.xpath("//div[contains(@style, 'background: rgb(45, 110, 199)')]//button")),
    10000
  );
