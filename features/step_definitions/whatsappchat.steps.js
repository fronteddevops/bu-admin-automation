...
Given(
  "I am logged in as Admin For Whatsapp",
  { timeout: 180000 },
  async function () {
    await driver.get(
...
When("Click on Customer", async function () {
  const customersButton = await driver.wait(
    until.elementLocated(By.xpath("//button[contains(text(), 'CUSTOMERS')]")),
    10000
  );
...
When("Attache Contact", async function () {
  // Open the dropdown
  await driver.findElement(By.css(".css-13cymwt-control")).click();

  // Wait and select the first item (index 0)
...
When("Select Contact", async function () {
  await driver.findElement(By.css(".css-13cymwt-control")).click();

  // Step 2: Wait until the option "Sandeep Sir Applatus" is visible
  const targetOption = await driver.wait(
    until.elementLocated(
      By.xpath(
        "//div[contains(@class,'option') and text()='Sandeep Sir Applatus']"
      )
    ),
    5000
  );
...
When("Click on Save", async function () {
  const saveButton = await driver.wait(
    until.elementLocated(By.xpath("//button[normalize-space()='Save']")),
    5000
  );

  // Wait until clickable
  await driver.wait(until.elementIsVisible(saveButton), 3000);
  await driver.wait(until.elementIsEnabled(saveButton), 3000);
...
When("Click on WhatsApp", async function () {
  const whatsappButton = await driver.wait(
    until.elementLocated(By.xpath("//button[normalize-space()='WHATSAPP']")),
    5000
  );
